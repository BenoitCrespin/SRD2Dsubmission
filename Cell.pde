class Cell {
  ...
  ArrayList<Particle> listParticles = new ArrayList<Particle>();
  ...
  void computePositionInFluidAndDensity () {
    ...
    densityRatio = listParticles.size();
    float nbNonEmpty = (float)gamma;
    if (listParticles.size() == 0) positionInFluid = isEmpty;
    else
      for (int ipos = -1; ipos <= 1; ipos++)
        for (int jpos = -1; jpos <= 1; jpos++) {
          ...
          if (cj.listParticles.size() == 0) positionInFluid = hasOneEmptyNeighbor;
          densityRatio += cj.listParticles.size();
  ...
  void computeAlphaAndAvgVel () {
    ...
    nbParticlesInCell = listParticles.size();
    for (Particle p : listParticles) avgVelocity.add(p.velocity);    
    if (nbParticlesInCell > 0) avgVelocity.div(nbParticlesInCell);

    computePositionInFluidAndDensity ();
    
    final float maxViscosity = 0;               // Use values between 0 and PI to set viscosity
    alpha = random(-maxViscosity, maxViscosity);
  }
