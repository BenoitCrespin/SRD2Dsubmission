class Particle {
  ...
  void applyPressureAndAdvect() {
    ...
    // Add to list of past positions
    pastPositions.add(curPos);
    if (pastPositions.size() > 10) pastPositions.remove(0);
  }
  ...
  void display() {
    ...
    for (PVector p: pastPositions) avgPos.add(p);
    avgPos.div(pastPositions.size());
    ellipse(avgPos.x, avgPos.y, 4, 4);
  }
