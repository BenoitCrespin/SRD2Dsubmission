...
// Press 'h' to print help in the application
final int   a_0 = 20;
final float gamma = 5;
...
final int   nbJacobiIterations = 10;
      ...
      float alpha = 0;                          // Use values between 0 and PI to set viscosity
...
void printHelp() {
  ...
  println("      'a/A': change alpha value");
...
void draw() {
  ...
  if ((keyPressed) && (frameCount > lastFrameKeyPressed + 40)) {
    ...
    if (key == 'h') printHelp();
    else if ((key == ' ') && (! vortexMode)) {
      addDroplet();   // SPACE: add droplet
    }
      ...
      if ((key == 'a') && (alpha > 0)) alpha -= PI/4;
      if ((key == 'A') && (alpha < PI)) alpha += PI/4;
      println("alpha = " + alpha);
    }
  }
